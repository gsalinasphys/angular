...
def get_epsilon_func(back: np.ndarray, params: dict) -> Callable:
    ...
    return lambda N: -derivative(H, N, dx=1e-5) / H(N)
...
def get_phi_double_prime_func(back: np.ndarray, params: dict) -> Callable:
    ...
    return lambda N: derivative(phiprime, N, dx=1e-5)
...
if __name__ == '__main__':
    ...
    plt.plot(Nplot, [H_func(_) for _ in Nplot], c="k", linewidth=2)
    plt.plot(Hs[:, 0], Hs[:, 1], c="b", linewidth=2, linestyle='--')
    ...
    plt.plot(Nplot, [epsilon_func(_) for _ in Nplot], c="k", linewidth=2)
    plt.plot(epsilons[:, 0], epsilons[:, 1], c="b", linewidth=2, linestyle='--')
    ...
    plt.plot(Nplot, np.abs([etapll_func(_) for _ in Nplot]), c="k", linewidth=2)
    plt.plot(back[:, 0], np.abs(etaplls), c="b", linewidth=2, linestyle='--')
    ...
    plt.plot(Nplot, [etaperp_func(_) for _ in Nplot], c="k", linewidth=2)
    plt.plot(back[:, 0], etaperps, c="b", linewidth=2, linestyle='--')
